...
def zeroTermination(b, offset=0, maxLen=None):
    i = offset
...
class BannerFile(object):
    META_DATA_SIZE = 0x140

    class MetaData(object):
        def __init__(self, data, offset=0):
            self.gameName = zeroTermination(data, offset+0x0, 0x20)
            self.developerName = zeroTermination(data, offset+0x20, 0x20)
            self.fullGameTitle = zeroTermination(data, offset+0x40, 0x40)
            self.fullDeveloperName = zeroTermination(data, offset+0x80, 0x40)
            self.gameDescription = zeroTermination(data, offset+0xc0, 0x80)
    ...
    def __init__(self, data): # data = bytes or InternalFile
        ...
        metaDataCount = (len(data) - metaDataOffset) // BannerFile.META_DATA_SIZE
        if metaDataCount == 1:
            self.meta = BannerFile.MetaData(data, metaDataOffset)
        else:
            self.meta = []
            for i in range(metaDataCount):
                offset = metaDataOffset + BannerFile.META_DATA_SIZE * i
